...
1. 以下のフォルダおよびファイルをダウンロードし、準備します。なお、`setting.json`や`.latexmkrc`は不要です。
   ...
   - `figures/`フォルダ
   - `data/`フォルダ（使用する場合）
2. CloudLaTeXのプロジェクト画面に、上記のファイル・フォルダをそのままドラッグ＆ドロップし、アップロードします。
3. 画面右上のハンバーガーメニューを開き、プロジェクト設定のLaTeXエンジンを`LuaLaTeX`に変更し、保存します。
4. `コンパイル`ボタンを押し、LaTeXのコンパイルを行います。
5. コンパイルが成功したら、以降は通常通り`main.tex`や`body.tex`を編集して使用してください。
